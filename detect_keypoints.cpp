...
void detKeypoints1()
{
    ...
    fast->detect(img, fastKeypoints);
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();

    cout << "Fast with n= " << fastKeypoints.size() << " keypoints in " << 1000 * t / 1.0 << " ms" << endl;

    cv::Mat fastImage= img.clone();
    drawKeypoints(img, fastKeypoints, fastImage, cv::Scalar::all(-1), cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);


    string windowName1="Fast Result";
    cv::namedWindow(windowName1, 2);
    imshow(windowName1, fastImage);
